...
final class LookMaxViewModel {
  ...
  func generatePlan(from assessment: LookMaxAssessment) {
    ...
    var overview =
      "Based on your goal for \(primaryArea.rawValue.lowercased()), we've designed a routine that fits your \(assessment.lifestyleLevel?.rawValue.lowercased() ?? "active") lifestyle. It respects your budget and time constraints."
    ...
    if primaryArea == .jawline || secondaryAreas.contains(.jawline) || primaryArea == .posture
      || secondaryAreas.contains(.posture)
    {
      dailyHabits.append(
        LookMaxAction(
          area: .posture,
      ...
      LazyVStack(alignment: .leading, spacing: 24, pinnedViews: [.sectionHeaders]) {
        ...
        VStack(alignment: .leading, spacing: 16) {
          ...
          VStack(alignment: .leading, spacing: 12) {
            ...
            Text(
              "EverForm is not a medical service. For medical or surgical questions, consult a professional."
            )
        ...
        if step != .intro {
          HStack(spacing: 4) {
            ForEach(0..<7) { index in
              Rectangle()
                .fill(
                  index <= stepIndex ? DesignSystem.Colors.accent : DesignSystem.Colors.neutral200
                )
                .frame(height: 4)
                .clipShape(Capsule())
            }
        ...
        ScrollView {
          VStack(spacing: 24) {
            ...
            case .photos:
              PhotosStep(
                assessment: Binding(
                  get: { viewModel.currentAssessment ?? LookMaxAssessment() },
                  set: { viewModel.currentAssessment = $0 }),
...
struct IntroStep: View {
  ...
  var body: some View {
    VStack(spacing: 24) {
      ...
      Text(
        "Upload a few photos, tell us what you want to change, and we’ll design a realistic, healthy Look Max roadmap."
      )
...
struct PhotosStep: View {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 20) {
      ...
      LookMaxPhotoCard(
        title: "Front View", caption: "Neutral face, natural lighting.",
        imageData: assessment.frontPhotoData, item: $frontItem)
      LookMaxPhotoCard(
        title: "Side View", caption: "Turn 90°, relaxed jaw.", imageData: assessment.sidePhotoData,
        item: $sideItem)
      LookMaxPhotoCard(
        title: "Back View (Optional)", caption: "For posture/back goals.",
        imageData: assessment.backPhotoData, item: $backItem)
    }
    .padding(.horizontal, 20)
  }
}
    ...
    VStack(alignment: .leading, spacing: 20) {
      ...
      LazyVGrid(columns: [GridItem(.adaptive(minimum: 100), spacing: 12)], spacing: 12) {
        ForEach(LookMaxArea.allCases) { area in
          let isSelected =
            assessment.primaryGoalArea == area || assessment.secondaryAreas.contains(area)
          Button {
            ...
            .background(
              isSelected
                ? DesignSystem.Colors.accent.opacity(0.1) : DesignSystem.Colors.cardBackground
            )
            .foregroundStyle(
              isSelected ? DesignSystem.Colors.accent : DesignSystem.Colors.textPrimary
            )
            .overlay(
              RoundedRectangle(cornerRadius: 12).stroke(
                isSelected ? DesignSystem.Colors.accent : Color.clear, lineWidth: 2)
            )
            .clipShape(RoundedRectangle(cornerRadius: 12))
          }
        }
    ...
    VStack(alignment: .leading, spacing: 24) {
      ...
      VStack(alignment: .leading, spacing: 8) {
        Text("What bothers you right now?")
          .font(DesignSystem.Typography.headline())
        TextField(
          "e.g., acne on cheeks, weak jawline", text: $assessment.mainConcern, axis: .vertical
        )
        .lineLimit(3...4)
        .padding()
        .background(DesignSystem.Colors.backgroundSecondary)
      ...
      VStack(alignment: .leading, spacing: 8) {
        Text("Desired outcome")
          .font(DesignSystem.Typography.headline())
        TextField(
          "e.g., clearer skin, sharper definition", text: $assessment.desiredOutcome,
          axis: .vertical
        )
        .lineLimit(3...4)
        .padding()
      ...
      VStack(alignment: .leading, spacing: 8) {
        Text("Notes (Optional)")
          .font(DesignSystem.Typography.headline())
        TextField(
          "Anything you absolutely DON'T want to do?", text: $assessment.notes, axis: .vertical
        )
        .lineLimit(2...3)
        .padding()
        .background(DesignSystem.Colors.backgroundSecondary)
    ...
    VStack(alignment: .leading, spacing: 24) {
      ...
      VStack(alignment: .leading, spacing: 8) {
        ...
        Picker("Gender", selection: $assessment.genderPresentation) {
          ...
          ForEach(GenderPresentation.allCases) { gender in
      ...
      ScrollView {
        VStack(alignment: .leading, spacing: 24) {
          ...
          EFCard(
            style: .gradient(
              LinearGradient(
                colors: [Color.black, Color(hex: "1C1C1E")], startPoint: .top, endPoint: .bottom))
          ) {
            VStack(alignment: .leading, spacing: 16) {
              Text("Ready for your glow up?")
                ...
                .foregroundStyle(.white)
              Text(
                "Upload photos, tell us what you want to improve, and we’ll build a personalized, healthy plan."
              )
              .font(DesignSystem.Typography.bodyMedium())
              ...
              EFCard {
                VStack(alignment: .leading, spacing: 8) {
                  Text(lastPlan.summaryTitle)
                    .font(DesignSystem.Typography.headline())
                  Text(
                    "\(lastPlan.dailyHabits.count) Daily Habits • \(lastPlan.weeklyActions.count) Weekly"
                  )
                  .font(DesignSystem.Typography.caption())
                  .foregroundStyle(DesignSystem.Colors.textSecondary)
                }
