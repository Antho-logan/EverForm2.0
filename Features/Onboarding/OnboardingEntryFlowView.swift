    ...
    VStack(spacing: 0) {
      ...
      HStack {
        if store.currentQuestion != .basicName {
          Button {
            ...
            Image(systemName: "chevron.left")
              ...
              .padding(12)
              .background(Circle().fill(EverFormTheme.Colors.surface))
          }
        } else {
        ...
        Text("Step \(store.currentQuestion.rawValue + 1) of \(OnboardingQuestion.allCases.count)")
          ...
          .padding(.vertical, 6)
          .background(Capsule().fill(EverFormTheme.Colors.surface))
    ...
    VStack(spacing: 32) {
      ...
      if optionsVisible {
        VStack(spacing: 24) {
          ...
          // Next Button
          EFPrimaryButton(
            question == .dietaryApproach ? "Finish" : "Next",
            action: onNext
          )
          .padding(.top, 16)
          .disabled(!isValid)
    ...
    case .height:
      HStack {
        TextField("175", value: Bindable(store).draft.heightCm, format: .number)
          ...
          .multilineTextAlignment(.center)
        Text("cm").font(EverFormTheme.Typography.sectionTitle)
      }
    ...
    case .weight:
      HStack {
        TextField("70", value: Bindable(store).draft.weightKg, format: .number)
          ...
          .multilineTextAlignment(.center)
        Text("kg").font(EverFormTheme.Typography.sectionTitle)
      }
...
struct OnboardingTextFieldStyle: TextFieldStyle {
  func _body(configuration: TextField<Self._Label>) -> some View {
    configuration
      ...
      .cornerRadius(EverFormTheme.Radius.small)
      .overlay(
        RoundedRectangle(cornerRadius: EverFormTheme.Radius.small)
          .stroke(EverFormTheme.Colors.divider, lineWidth: 1)
      )
  }
}
