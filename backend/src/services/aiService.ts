...
import { OnboardingAnswer, PersonalPlan, PillarRecentData, Profile } from '../types';

const DEEPSEEK_URL = 'https://api.deepseek.com/v1/chat/completions';

function buildMockPlan(profile: Profile | null, recentData: PillarRecentData): PersonalPlan {
  return {
    trainingPlan: {
...
export async function generatePersonalPlan(
  ...
  recentData: PillarRecentData
): Promise<PersonalPlan> {
  const systemPrompt =
    'You are EverForm, a biohacking and fitness coach. Using the structured JSON user profile, questionnaire answers, and recent activity, generate a 12-week personalized plan that includes training, nutrition, recovery, mobility, pain-prevention, breathwork, and look-maxing guidance. Respond with strict JSON only.';
  ...
  try {
    ...
    if (content) {
      try {
        return JSON.parse(content) as PersonalPlan;
      } catch (parseErr) {
