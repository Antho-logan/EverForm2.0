...
import { analyzeImage, ScanResponse } from '../services/scanService';
import { analyzePlateImage } from '../services/plateService';
import { AuthenticatedRequest } from '../types';
import { userInsert } from '../utils/db';
...
const FALLBACK_RESULT: ScanResponse = {
  mode: 'calories',
  calories: 500,
  protein: 25,
  carbs: 50,
...
router.post('/plate-image', upload.single('image'), async (req, res) => {
  ...
  try {
    ...
    const result = await analyzePlateImage(imageBase64, mimetype);
 ...
 */
router.post('/meal', async (req: AuthenticatedRequest, res) => {
  try {
    ...
    try {
      result = await analyzeImage(parseResult.data);
    } catch (analysisErr) {
