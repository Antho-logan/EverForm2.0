...
extension View {
  func efCardStyle(scheme: ColorScheme) -> some View {
    ...
    return
      ...
      .shadow(
        color: scheme == .dark ? Theme.Shadow.card : .black.opacity(0.06), radius: 8, x: 0, y: 4)
  }
}
...
public struct EFBackground: ViewModifier {
  ...
  public func body(content: Content) -> some View {
    content
      .background(
        Group {
          if scheme == .dark { EFColor.bgDark } else { EFColor.bgLight }
        }
        .ignoresSafeArea()
      )
  }
}
extension View { public func efBackground() -> some View { modifier(EFBackground()) } }
